...
class EditSalidasScreen extends StatefulWidget {
  const EditSalidasScreen._({required this.salida});
  static Widget init({required Salidas salida}) => ChangeNotifierProvider(
        create: (context) =>
            SalidasProvider(salidasRepo: context.read<SalidaRepository>()),
        child: EditSalidasScreen._(
          salida: salida,
        ),
      );
  final Salidas salida;
  @override
  State<EditSalidasScreen> createState() => _EditSalidasScreenState();
}

List<String> tipoSalida = ['dirigida', 'práctica'];

class _EditSalidasScreenState extends State<EditSalidasScreen> {
  ...
  @override
  void initState() {
    final salida = widget.salida;
    fecha = salida.fecha;
    selectedTipo = salida.tipo;
    salidaLugar.text = salida.lugar;
    salidaCupos.text = salida.cupos.toString();
    salidaPrecios.text = salida.precio.toString();
    salidaLugarSalida.text = salida.lugarDeSalida.toString();
    super.initState();
  }
        ...
        body: Column(children: [
          ...
          Padding(
            ...
            child: TextButton.icon(
                onPressed: () {
                  context.read<SalidasProvider>().edit(Salidas(
                      id: widget.salida.id,
                      fecha: fecha,
